...
set.seed(778)
three_old = sample(exam_questions, 3)
twenty_retake_1 = setdiff(2*(1:23) - 1, black_list) # odd numbers not in black list
set.seed(778)
fifteen_retake = sample(twenty_retake_1, 15)
new = setdiff(2*(24:35) - 1, black_list)

retake2_questions = c(three_old, fifteen_retake, new)
files_sample = files_all[retake2_questions]
...
paths = paste0("pdf_ret2_0", 1:4, "/the_exam.rds")
paths

all_data = tibble(paths = paths) %>% mutate(ans_data = map(paths, data_2_answers))
all_data_long = unnest(all_data)
rio::export(all_data_long, "retake2_answers.xlsx")
