...
\begin{enumerate}[resume]
	...
	%Задача №1
	\item[6.] Василиса Премудрая в рамках борьбы с гендерным дресс-кодом в Тридевятом Царстве устроила распродажу всех своих пяти
  ...
  если она продаёт кокошники по себестоимости и вероятность покупки любого из них одна и та же. \textbf{(5 баллов)}
	\item[] Решение: \par 
	Пусть $X_1, ..., X_5$ - цены кокошников. $\E (X_i) = 3500, \Var (X_i) = 500^2 = 250000, \ i = 1, ..., 5.$ 
\par Василиса выбирает наугад и равновероятно 3 кокошника, обозначим их цены за $Y_1, Y_2, Y_3$. \par 
Не трудно догадаться, что матожидание вырученных Василисой денег - это  \par $\E (Y_1 + Y_2 + Y_3) = 
\E (Y_1)  + \E (Y_2) + \E (Y_3) = 3 \cdot 3500 = 10500.$
\par Аналогично, дисперсия вырученных Василисой денег - это $\Var (Y_1 + Y_2 + Y_3)$ \par
$\Var (Y_1 + Y_2 + Y_3) = \Var (Y_1) + \Var (Y_2) + \Var (Y_3) + 2 \cov (Y_1, Y_2) + 2 \cov (Y_1, Y_3) + 2 \cov (Y_2, Y_3)$
\par Найдем ковариации соответствующих случайных величин, используя следующее свойство: 
\par $ \cov (X_1, X_1 + X_2 + ... + X_5) = 0$, так как $X_1 + X_2 + ... + X_5 = const$ 
\par $ \cov (X_1, X_1 + X_2 + ... + X_5) = \Var (X_1) + 4 \cov (X_1, X_i) = 0,$ \par так 
как ковариации $X_1$ со всеми остальными $X_i$ будут одинаковы. \par Отсюда получаем 
$\cov (X_1, X_i) = - \frac{1}{4} \cdot \Var (X_1)$ \par
Получим: $ \Var (Y_1 + Y_2 + Y_3) = 3 \cdot \Var (Y_1) - 3 \cdot 2 \cdot \frac{1}{4} \cdot \Var (Y_1) =
