...
\item Нетрудно убедиться, что функция плотности $\xi_{1}$ не будет совпадать с условной при некоторых значениях $\xi_{2}$, 
что докажет отсутствие независимости:
...
\[
	\lim_{n\to\infty} \E(\xi_{n})=\lim_{n\to\infty}n^3 \cdot \frac{1}{n}+0\cdot (1-\frac{1}{n})=\lim\limits_{n\to\infty} n^2=\infty
\]
...
\begin{enumerate}
\item Пусть случайная величина $X\sim \cBin(n=1000, p=0.02)$ отражает количество страховых случаев и имеет биномиальное распределение с параметрами $n=1000$ и $p=0.02$.

Оценим соответствующую вероятность при помощи неравенства Маркова:

\[
	\P(X\leq20) = 1 - \P(X\geq 20)\geq 1 - \frac{\E(X)}{20} 
	= 1 - \frac{1000 \cdot 0.02}{20} = 0
\]
...
\[
	X\dot\sim \cN(1000\cdot 0.02, 1000\cdot (0.02)\cdot (1-0.02)) \sim \cN(20,19.6)
\]
...
\[
	\E(Y)=\E(25 X)=25\E(X)=25 \cdot 20=500
\]

\[
	\Var(Y)=\Var(25 X)=25^2 \Var(X) = 25^2\cdot 20 \cdot (1-0.02) = 12250
\]

Пользуясь ЦПТ получаем:

\[
	Y\dot\sim \cN(500, 12250)
\]
...
\[
	\P(Y\geq S)=\Phi\left(\frac{S-500}{\sqrt{12250}}\right)=0.9
\]

Пользуясь квантилью стандартного нормального распределения имеем:

\[
	\P(Y\geq S)=\frac{S-500}{\sqrt{12250}}\approx1.28
\]

Решая получаем, что $S \approx 640$.
