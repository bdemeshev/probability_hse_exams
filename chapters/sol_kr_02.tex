...
\item[8.]
\begin{enumerate}
\item Выпишем случайную величину $X_i$ — цену акции после $i$-го дня:
\[
X_i =
\begin{cases}
1.01, & p = 0.7 \\
...
Пусть случайная величина $\xi_i$ означает число посетителей сайта за $i$-й день. По условию, $\xi_i \sim Pois(\lambda=250)$. Известно, что если $\xi \sim Pois(\lambda)$, то $\E(\xi) = \Var(\xi) = \lambda$.
...
\[
X_i = \begin{cases}
1, & \text{если на i-м шаге Винни-Пух пошёл направо} \\
-1, & \text{если пошёл налево}
\end{cases}
\quad i=1,\ldots, n;
\]
Тогда $S_n := X_1 +\ldots+X_n$ означает местоположение Винни-Пуха в $n$-ю минуту его блужданий по прямой.
...
X_i =
\begin{cases}
1, \text{ если }i\text{-й ребёнок — мальчик }\\
0, \text{ иначе }
\end{cases}
\]
...
Пусть $Z$ — число правильных ответов на вопросы с 3-го по 7-й (у Пети и у Васи)
\begin{align*}
...
(здесь $\bar{X}_{i}$ — среднее арифметическое по $i$-й страте)
...
индикатор того, что $i$-й ребенок — мальчик, и $F_{i}$ — индикатор того, что
$i$-й ребенок — девочка. Конечно, $F_{i}+M_{i}=1$ и $F_{i}M_{i}=0$.
