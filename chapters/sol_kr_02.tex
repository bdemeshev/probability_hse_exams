...
\item В данном пункте требуется найти сумму двух независимых случайных величин, 
что можно сделать при помощи формулы свертки:
...
Можно рассчитать данное математическое ожидание и напрямую:

\[
	\int_{0}^{2}\int_{0}^{2}(x-y)^{2019} \, dxdy 
	= 
	\int_{0}^{2}\left(\frac{(2 - y)^{2020} - y^{2020}}{2020}\right) \, dy = 0
\]
...
\[
	...
	=
	\frac{\frac{(2-0)^2}{12}-0}{\sqrt{\frac{2}{3} - \frac{(2-0)^2}{12}}} 
	= 
	\frac{1}{\sqrt{3}}
\]
...
Данные случайные величины не являются независимыми даже несмотря на то, что их ковариация равняется нулю, поскольку, например:

\[
F_{\begin{bmatrix}\xi_{1}-\xi_{2}\\ \xi_{1}+\xi_{2}\end{bmatrix}}(0,4)=0\ne\frac{1}{2}*1=F_{\xi_{1}-\xi_{2}}(0)F_{\xi_{1}+\xi_{2}}(4)
\]
...
\item Исходя из результатов, полученных в предыдущем пункте, очевидно, что:

\[E(\xi_{2}^3)=\int_{0}^{1}y^3*3y^2dy=\frac{1}{2}\]

\item Очевидно, что:

\[\int\limits_{0}^{1}\int\limits_{0}^{y}\left(\frac{x}{y}*3y\right)dxdy=\frac{1}{2}\]

\item Действуя строго по определению получаем:

\[f_{\xi_{1}|\xi_{2}=y}(x)=\frac{f_{\xi}(x,y)}{f_{\xi_{2}}(y)}=\begin{cases}\frac{3y}{3y^2}=\frac{1}{y}\text{, если }x,y\in(0,1)\wedge y>x\\0\text{, в противном случае}\end{cases}\]

\item Пользуясь полученным в предыдущем пункте результатом имеем:

\[E(\xi_{1}|\xi_{2}=y)=\int_{0}^{y}x*\frac{1}{y}dx=\frac{y}{2}\]
...
\[
	f_{E(\xi_{1}|\xi_{2})}(t)=\frac{dF_{\xi_{2}}(2t)}{dt}=2f_{\xi_{2}}(2t)=\begin{cases}2*3(2t)^2=24t^2\text{, если }t\in(0,0.5)\\0\text{, в противном случае}\end{cases}
\]

\item Очевидно, что:

\[E(E(\xi_{1}|\xi_{2}))=E(\xi_{1})=\int_{0}^{1}\int_{0}^{y} x*3y dxdy=\frac{3}{8}\]

\item Нетрудно убедиться, что маржинальная функция плотности $\xi_{1}$ не будет совпадать с условной при некоторых значениях $\xi_{2}$, что докажет отсутствие независимости:

\[f_{\xi_{1}}(0.5)=\int_{0.5}^{1}3y dy=\frac{3}{2}(1-0.5^2)=1.125\ne2=f_{\xi_{1}|\xi_{2}=0.5}(0.5)\]

\item Нетрудно догадаться, что:

\[\int_{0}^{1}\int_{0}^{y}cx*3y=\frac{3}{8}c=1\]
...
\[\lim\limits_{n\to\infty}P(|\xi_{n}-0|>\epsilon)\leq \lim\limits_{n\to\infty}P(|\xi_{n}-0|>0)=\lim\limits_{n\to\infty}\frac{1}{n}=0\]

\item Очевидно, что

\[\lim\limits_{n\to\infty} E(\xi_{n})=\lim\limits_{n\to\infty}n^3*\frac{1}{n}+0*(1-\frac{1}{n})=\lim\limits_{n\to\infty} n^2=\infty\]
...
\begin{enumerate}
\item Пусть случайная величина $X\sim B(1000,0.02)$ отражает количество страховых случаев и имеет биномиальное распределение с параметрами $n=1000$ и $p=0.02$.

Оценим соответствующую вероятность при помощи неравенства Маркова:

\[P(X\leq20)=1-P(X\geq20)\geq1-\frac{E(X)}{20}=1-\frac{1000*0.02}{20}=0\]

Теперь воспользуемся неравенством Чебышёва (\textbf{для варианта k}):

\[P(X\leq20)=P(|X-10|\leq10)=1-P(|X-10|\geq10)\geq1-\frac{1000*0.01*(1-0.01)}{10^2}=0.901\]

Пользуясь ЦПТ получаем, что:

\[X\dot\sim N(1000*0.02,1000*(0.02)*(1-0.02) => X\dot\sim N(20,19.6)\]

В силу симметрии нормального распределения вокруг математического ожидания получаем, что:

\[P(X\leq20)=\frac{1}{2}\]

\item Найдем математическое ожидание и дисперсию выплат $Y$:

\[E(Y)=E(25000 X)=25000E(X)=25000*20=500000\]

\[Var(Y)=Var(25000 X)=25000^2Var(X)=25000^2*20*(1-0.02)=12250000000\]

Пользуясь ЦПТ получаем:

\[Y\dot\sim \mathcal{N}(500000, 12250000000)\]
...
\[P(Y\geq S)=\Phi\left(\frac{S-500000}{\sqrt{12250000000}}\right)=0.9\]

Пользуясь квантилью стандартного нормального распределения имеем:

\[P(Y\geq S)=\frac{S-500000}{\sqrt{12250000000}}\approx1.281552\]

Решая получаем, что $S\approx641842$.
...
\item 
	Пусть $T$ — это случайная величина равная $T_{1} + T_{2}$.	
\begin{enumerate}
	\item  
