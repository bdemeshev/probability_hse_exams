...
\end{enumerate}
\item \begin{enumerate}
  \item Критерий Колмогорова с классическими критическими значениями годится для непрерывного распределения.
  Примерно непрерывное распределение, а именно, $\cN(0, 1)$, будет иметь стандартизированная доля,
  \[
  Z_i = (\hat p_i - 0.75) / \sqrt{0.25 \cdot 0.75 / n_i}.
  \]
  Находим три значения по трём ящикам, $\hat p_1 = 120/150$, $Z_1 = 1.41$, 
  $\hat p_2 = 60/100$, $Z_2 = -3.46$, $\hat p_3 = 95/120$, $Z_3 = 1.05$.
  Значения нормальной функции распределения равны $F(Z_1) = 0.92$, $F(Z_2) = 0$, $F(Z_3) = 0.85$.

  Отсюда,
  \[
  \sup \abs{F(t) - \hat F(t)}  = 0.85 - 1/3 \approx 0.52
  \]
